...
-- ============================================================
-- V9 CHIUSURA: ROSA, SEGRETARI, COMUNICAZIONI/CONVOCAZIONI
-- Non modifica distinte, calendario, classifiche o risultati.
-- ============================================================
...
create unique index if not exists uq_team_role_user
 on public.team_user_roles(team_id,user_id,role);

create unique index if not exists uq_callup_player
 on public.team_callup_players(callup_id,player_id);
...
create function public.captain_appoint_secretary_v2(
 p_team_id uuid,
 p_email text
)
returns boolean
...
begin
 ...
 if not exists(
  ...
  from public.teams
  where id=p_team_id
    and lower(trim(captain_email))=v_current_email
 )
 and not exists(
  select 1
 ...
 then
  raise exception 'Solo il capitano può nominare il segretario.';
 end if;
 ...
 if (
  ...
  raise exception 'Puoi nominare al massimo 3 segretari.';
 end if;
 ...
 if v_user is null then
  raise exception 'Nessun account trovato con questa email.';
 end if;
...
create function public.captain_revoke_secretary(
 p_team_id uuid,
 p_role_id uuid
)
returns boolean
...
begin
 ...
 then
  raise exception 'Solo il capitano può revocare un segretario.';
 end if;
...
begin
 ...
 and not exists(
  select 1 from public.teams
  where id=p_team_id
    and lower(trim(captain_email))=v_email
 )
 then
  raise exception 'Account non autorizzato per questa squadra.';
 end if;
     ...
     select coalesce(
      jsonb_agg(
       jsonb_build_object(
        ...
        'player_name',concat(r.first_name,' ',r.last_name),
        'response',cp.response,
        'response_note',cp.response_note
       )
      ),
      '[]'::jsonb
     )
...
create function public.captain_send_team_message(
 p_team_id uuid,
 p_body text
)
returns uuid
...
begin
 ...
 insert into public.team_messages(
  id,team_id,sender_user_id,sender_role,sender_name,body
 )
 values(
  v_id,p_team_id,auth.uid(),'captain',
  coalesce(v_name,'Capitano'),trim(p_body)
 );
...
begin
 ...
 if jsonb_array_length(coalesce(p_player_ids,'[]'::jsonb))=0 then
  raise exception 'Seleziona almeno un giocatore.';
 end if;

 insert into public.team_callups(
  id,fixture_id,team_id,created_by,title,message,deadline
 )
 values(
  v_id,p_fixture_id,p_team_id,auth.uid(),
  'Convocazione partita',nullif(trim(p_message),''),p_deadline
 );

 for v_player in
  select * from jsonb_array_elements(p_player_ids)
 loop
  insert into public.team_callup_players(callup_id,player_id)
  values(v_id,(v_player#>>'{}')::uuid)
...
begin
 ...
 if v_player_id is null then
  raise exception 'Profilo giocatore non collegato.';
 end if;
 ...
 into v_callups
 from public.team_callups c
 join public.team_callup_players cp on cp.callup_id=c.id
...
begin
 ...
 values(
  v_id,v_player.team_id,auth.uid(),'player',
  concat(v_player.first_name,' ',v_player.last_name),
  trim(p_body)
 );
...
create function public.player_respond_callup(
 p_callup_id uuid,
 p_response text,
 p_note text
)
...
begin
 ...
 update public.team_callup_players
